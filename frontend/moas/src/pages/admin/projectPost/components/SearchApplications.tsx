// src/pages/admin/projectPost/components/SearchApplications.tsx
...
export const SearchApplications = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const data = await getArtistApplications(params);
      setApplications(data.content);
      setCurrentPage(1);
      setTotalPages(data.pageInfo.totalPages);
    } catch (err) {
      setError('지원 검색에 실패했습니다.');
      console.error('지원 검색 실패:', err);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <>
      <div>
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">지원 검색</h3>
